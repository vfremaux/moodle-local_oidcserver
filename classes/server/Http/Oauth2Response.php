 ...
 */
class OAuth2Response extends Response implements ResponseInterface
{
    public $version;
    ...
    public function withSatus($status, $reasonPhrase = '') {
        $response = clone($this);
        $response->setStatuCode($status);
        if (!empty($reasonPhrase)) {
            $response->statusText = $reasonPhrase;
        } else {
            $response->statusText = self::statusTexts[$status];
        }
        return $response;
    }
